...
const IndicateModal: React.FC<IndicateModalProps> = ({ post, currentUser, onClose, onPostUpdated }) => {
  ...
  const followers = currentUser.followedUsers
    .map(id => findUserById(id))
    .filter(u => u && `${u.firstName} ${u.lastName}`.toLowerCase().includes(searchTerm.toLowerCase()));

  const handleIndicate = (targetUserId: string) => {
    const success = indicatePostToUser(post.id, currentUser.id, targetUserId);
    if (success) {
      setIsSuccess(true);
      onPostUpdated();
